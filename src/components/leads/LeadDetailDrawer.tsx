...
export function LeadDetailDrawer({ lead, onClose, currentUserName, onLeadUpdated }: LeadDetailDrawerProps) {
  ...
  async function handleAddNote(e: React.FormEvent) {
    ...
    setSavingNote(true);
    const { error } = await supabase.from('lead_activities').insert({ lead_id: lead.id, type: 'System Update', note: newNote.trim(), logged_by: currentUserName });
    if (!error) { setNewNote(''); await fetchActivities(); }
    setSavingNote(false);
  }
  ...
  async function handleLogActivity(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('lead_activities').insert({ lead_id: lead.id, type: activityType, note: activityNote.trim(), logged_by: currentUserName });
    if (!error) { setActivityNote(''); await fetchActivities(); onLeadUpdated(); }
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700 flex-shrink-0 px-6">
          {(['profile', 'timeline', 'notes'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`py-3 px-1 mr-5 text-sm font-semibold border-b-2 transition-colors capitalize ${activeTab === tab ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400 dark:border-indigo-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
              {tab}
            </button>
          ))}
        </div>
          ...
          {activeTab === 'profile' && (
            <div className="p-6 space-y-6">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300">{lead.source}</span>
              </div>
              ...
              <div className="space-y-3">
                ...
                <InfoRow icon={<Clock className="w-4 h-4" />} label="Updated" value={formatRelativeTime(lead.updated_at)} />
              </div>
              {lead.notes && (
            ...
            <div className="p-6 space-y-6">
              {isActive && (
                <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700">
                  ...
                  <form onSubmit={handleLogActivity} className="space-y-3">
                    ...
                    <button type="submit" disabled={savingActivity || !activityNote.trim()}
                      ...
                      {savingActivity ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Saving...</> : <><Plus className="w-4 h-4" />Save Activity</>}
                    </button>
                  </form>
                </div>
              )}
              <div>
                ...
                {loadingActivities ? (
                  ...
                  <div className="text-center py-8 text-gray-400 dark:text-gray-500"><Clock className="w-8 h-8 mx-auto mb-2 opacity-40" /><p className="text-sm">No activities yet</p></div>
                ) : (
                    ...
                    <div className="space-y-4">
                      {activities.map(activity => (
                        <div key={activity.id} className="relative flex gap-3 pl-10">
                          <div className={`absolute left-2 -translate-x-1/2 w-5 h-5 rounded-full flex items-center justify-center ${ACTIVITY_COLORS[activity.type as ActivityType]}`}>
                          ...
                          <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-3 shadow-sm">
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <span className="text-[11px] text-gray-400">{formatRelativeTime(activity.created_at)}</span>
                            </div>
                            <p className="text-sm text-gray-600 dark:text-gray-400">{activity.note}</p>
                            <p className="text-[11px] text-gray-400 mt-1.5">By {activity.logged_by}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                ...
                {loadingActivities ? (
                  ...
                  <div className="text-center py-8 text-gray-400 dark:text-gray-500"><FileText className="w-8 h-8 mx-auto mb-2 opacity-40" /><p className="text-sm">No notes yet</p></div>
                ) : (
                  <div className="space-y-3">
...
function InfoRow({ icon, label, value, highlight }: { icon: React.ReactNode; label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center gap-3 py-2 border-b border-gray-50 dark:border-gray-800 last:border-0">
      ...
      <span className={`text-sm font-medium truncate ${highlight ? 'text-indigo-600 dark:text-indigo-400 font-bold' : 'text-gray-800 dark:text-gray-200'}`}>{value}</span>
    </div>
  );
}
...
export function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${styles[status] ?? 'bg-gray-100 text-gray-600'}`}>{status}</span>;
}
