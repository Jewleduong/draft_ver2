...
function getLastActivityType(activities: { type: string; created_at: string }[]) {
  if (!activities.length) return null;
  return activities.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())[0].type;
}
...
export function LeadsPage({ role, userName, onToast }: LeadsPageProps) {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    if (leadsData.length > 0) {
      ...
      const { data: actData } = await supabase.from('lead_activities').select('lead_id, type, created_at').in('lead_id', ids).order('created_at', { ascending: false });
  ...
  const filtered = leads
    .filter(l => {
      if (search) { const q = search.toLowerCase(); if (!l.name.toLowerCase().includes(q) && !l.company.toLowerCase().includes(q) && !l.email.toLowerCase().includes(q)) return false; }
  // Filter + sort
  const filtered = leads
    .filter(l => {
      if (search) {
  ...
  async function handleCreateLead(data: Omit<Lead, 'id' | 'created_at' | 'updated_at'>) {
    ...
    setModal(null); onToast('Lead created successfully'); await fetchLeads();
    setModal(null);
  ...
  async function handleEditLead(id: string, data: Partial<Lead>) {
    ...
    setModal(null); setSelectedLead(null); onToast('Lead updated');
    setModal(null);
  ...
  async function handleConvertLead(dealData: { name: string; value: number; stage: string; expected_close_date: string }) {
    ...
    const { error: dealErr } = await supabase.from('deals').insert({ lead_id: selectedLead.id, name: dealData.name, value: dealData.value, stage: dealData.stage, expected_close_date: dealData.expected_close_date, company: selectedLead.company, contact_person: selectedLead.name, contact_email: selectedLead.email, contact_phone: selectedLead.phone, owner_name: selectedLead.owner_name, probability: 25 });
    if (dealErr) { onToast('Failed to create deal'); return; }
    await supabase.from('leads').update({ status: 'Converted' }).eq('id', selectedLead.id);
    await supabase.from('lead_activities').insert({ lead_id: selectedLead.id, type: 'System Update', note: `Converted to deal: "${dealData.name}"`, logged_by: userName });
    setModal(null); setSelectedLead(null); onToast(`Lead converted to deal: ${dealData.name}`); await fetchLeads();
    ...
    const { error: leadErr } = await supabase.from('leads').update({ status: 'Converted' }).eq('id', selectedLead.id);
    if (leadErr) { onToast('Lead update failed'); return; }
  ...
  async function handleRejectLead(lead: Lead) {
    ...
    await supabase.from('lead_activities').insert({ lead_id: lead.id, type: 'System Update', note: 'Lead marked as Rejected', logged_by: userName });
    onToast('Lead rejected'); await fetchLeads();
  }
  ...
  async function handleDeleteLead(id: string) {
    ...
    setDeletingId(null); onToast('Lead deleted');
  ...
  function exportLeads() {
    const csv = [['Name', 'Company', 'Email', 'Phone', 'Source', 'Status', 'Value', 'Owner'].join(','), ...filtered.map(l => [l.name, l.company, l.email, l.phone, l.source, l.status, l.estimated_value, l.owner_name].join(','))].join('\n');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center gap-3">
          {isManager && <button onClick={exportLeads} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition shadow-sm"><Download className="w-4 h-4" />Export</button>}
          <button onClick={() => setModal('create')} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm transition"><Plus className="w-4 h-4" />Create Lead</button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
        <div className="flex flex-wrap gap-3">
          ...
          <button onClick={fetchLeads} className="p-2 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-400 hover:text-indigo-600 hover:border-indigo-400 bg-gray-50 dark:bg-gray-900 transition" title="Refresh"><RefreshCw className="w-4 h-4" /></button>
        </div>
      </div>

      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center py-20"><span className="w-8 h-8 border-2 border-indigo-200 border-t-indigo-600 rounded-full animate-spin" /></div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400 dark:text-gray-500">
            <Users className="w-12 h-12 mb-3 opacity-30" /><p className="text-base font-medium">No leads found</p>
      {/* Page Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-gray-200 dark:border-gray-700">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Lead Directory</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Manage incoming prospects, qualify opportunities, and track engagement history.
          </p>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0">
          {isManager && (
            <button
              onClick={exportLeads}
              className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition shadow-sm"
            >
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
        <div className="flex flex-wrap gap-3">
          ...
          <button
            onClick={fetchLeads}
            className="p-2 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-400 hover:text-indigo-600 hover:border-indigo-400 bg-gray-50 dark:bg-gray-900 transition"
            title="Refresh"
          >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-900/60 border-b border-gray-200 dark:border-gray-700">
                  {['Lead', 'Contact', 'Source', isManager ? 'Owner' : null, 'Value', 'Status', 'Last Activity', 'Contacted', ''].filter(Boolean).map(col => (
                    <th key={col} className={`text-left px-4 py-3 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider ${col === 'Contact' ? 'hidden md:table-cell' : ''} ${col === 'Source' || col === 'Owner' ? 'hidden lg:table-cell' : ''} ${col === 'Value' ? 'text-right hidden sm:table-cell' : ''} ${col === 'Last Activity' || col === 'Contacted' ? 'hidden xl:table-cell' : ''}`}>{col}</th>
                  ))}
                ...
                {filtered.map(lead => {
                  ...
                  return (
                    <tr key={lead.id} className="hover:bg-gray-50/80 dark:hover:bg-gray-700/40 transition-colors group">
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center text-xs font-bold flex-shrink-0">{lead.name.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase()}</div>
                          <div className="min-w-0"><p className="font-semibold text-gray-900 dark:text-white truncate max-w-[140px]">{lead.name}</p><p className="text-xs text-gray-500 dark:text-gray-400 truncate max-w-[140px]">{lead.company}</p></div>
                        </div>
                      </td>
                      ...
                      {isManager && <td className="px-4 py-3.5 hidden lg:table-cell"><span className="text-xs text-gray-600 dark:text-gray-400">{lead.owner_name}</span></td>}
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                          <ActionBtn onClick={() => setDrawerLead(lead)} title="View" className="hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/40"><Eye className="w-3.5 h-3.5" /></ActionBtn>
                          <ActionBtn onClick={() => { setSelectedLead(lead); setModal('edit'); }} title="Edit" className="hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-950/40"><Edit2 className="w-3.5 h-3.5" /></ActionBtn>
                          {isActive && <ActionBtn onClick={() => { setSelectedLead(lead); setModal('convert'); }} title="Convert" className="hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/40"><ArrowRightCircle className="w-3.5 h-3.5" /></ActionBtn>}
                          {isActive && <ActionBtn onClick={() => handleRejectLead(lead)} title="Reject" className="hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/40"><XCircle className="w-3.5 h-3.5" /></ActionBtn>}
                          {isManager && <ActionBtn onClick={() => handleDeleteLead(lead.id)} title="Delete" className="hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40" disabled={deletingId === lead.id}>{deletingId === lead.id ? <span className="w-3.5 h-3.5 border-2 border-red-300 border-t-red-600 rounded-full animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}</ActionBtn>}
                  const lastAct = getLastActivityType(activities[lead.id] ?? []);
                  const lastActDate = activities[lead.id]?.[0]?.created_at;
                  const isActive = lead.status !== 'Converted' && lead.status !== 'Rejected';
                  ...
                  return (
                    ...
                    >
                      ...
                      {isManager && (
                        <td className="px-4 py-3.5 hidden lg:table-cell">
                          <span className="text-xs text-gray-600 dark:text-gray-400 truncate max-w-[100px] block">{lead.owner_name}</span>
                        </td>
                      )}

                      {/* Value */}
                      <td className="px-4 py-3.5 text-right hidden sm:table-cell">
                        <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400">{formatCurrency(lead.estimated_value)}</span>
                      </td>
                      ...
                      <td className="px-4 py-3.5 hidden xl:table-cell">
                        {lastAct ? (
                          <span className="text-xs text-gray-500 dark:text-gray-400">{lastAct}</span>
                        ) : (
                      ...
                      <td className="px-4 py-3.5 hidden xl:table-cell">
                        {lastActDate ? (
                          <span className="text-xs text-gray-500 dark:text-gray-400">{formatDaysAgo(lastActDate)}</span>
                        ) : (
                          <span className="text-xs text-gray-300 dark:text-gray-600">—</span>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                          <ActionBtn
                            ...
                            title="View Details"
                            className="hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/40"
                          >
                            <Eye className="w-3.5 h-3.5" />
                          </ActionBtn>
                          <ActionBtn
                            onClick={() => { setSelectedLead(lead); setModal('edit'); }}
                            title="Edit Lead"
                            className="hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-950/40"
                          >
                            <Edit2 className="w-3.5 h-3.5" />
                          </ActionBtn>
                          {isActive && (
                            <ActionBtn
                              onClick={() => { setSelectedLead(lead); setModal('convert'); }}
                              title="Convert to Deal"
                              className="hover:text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/40"
                            >
                          ...
                          {isActive && (
                            <ActionBtn
                              ...
                              title="Reject Lead"
                              className="hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/40"
                            >
            ...
            <div className="px-4 py-3 border-t border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30 flex items-center justify-between">
              <p className="text-xs text-gray-400">
                Showing <span className="font-semibold text-gray-600 dark:text-gray-300">{filtered.length}</span> of <span className="font-semibold text-gray-600 dark:text-gray-300">{total}</span> leads
              </p>
            </div>
          </div>
        )}
...
function KpiCard({ title, value, icon, iconBg, sub, highlight }: {
  title: string;
  value: string | number;
  ...
  const valColor = highlight ? valueColors[highlight] : 'text-gray-900 dark:text-white';
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-5">
      ...
      <p className={`text-3xl font-extrabold ${valColor}`}>{value}</p>
...
function ActionBtn({ children, onClick, title, className, disabled }: {
  ...
  return (
    <button
      ...
      title={title}
      disabled={disabled}
      className={`p-1.5 rounded-lg text-gray-400 transition-colors ${className} disabled:opacity-50`}
    >
      {children}
    </button>
  );
